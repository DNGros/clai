...
class ProgSeq2SeqDataset(Dataset):
    def __init__(self, data: ACDataset, tokenizer, max_len_src=512, max_len_tgt=512):
    ...
    def _build(self):
        for ex in self.data.examples:
            src = ex.nl_norm
            target = cmd_to_seq(ex.cmd)
            # tokenize inputs
            tokenized_inputs = self.tokenizer.batch_encode_plus(
                [src], max_length=self.max_len_tgt, pad_to_max_length=True, return_tensors="pt"
            )
            # tokenize targets
